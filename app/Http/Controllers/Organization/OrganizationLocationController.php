...
use App\Domains\Organization\Locations\OrganizationLocationOptionData;
use App\Domains\Organization\Locations\OrganizationLocationResourceData;
use App\Domains\Organization\Locations\OrganizationLocationService;
...
{
    private string $componentDashboard = 'v1/org/location/dashboard';

    private string $componentManagement = 'v1/org/location/manage-location';

    private string $handlerRedirectTo = 'v1.org.locations.manage:get';

    public function __construct(private OrganizationLocationService $service) {}
            ...
            return inertia()->render($this->componentManagement, [
                'context' => [
                    'form' => ResponseFormData::forPrefixed(
                        data: OrganizationLocationResourceData::from($model)->toArray(),
    ...
    public function addHandler(OrganizationLocationFormData $data): ?RedirectResponse
    {
        try {
    ...
    public function updateHandler(string $prefixedId, OrganizationLocationFormData $data): ?RedirectResponse
    {
        try {
    ...
    public function datatable(DatatableRequestData $requestData): JsonResponse
    {
        try {
            ...
            return successResponseJson([
                'data' => OrganizationLocationResourceData::collect(items: $result),
            ]);
        } catch (Exception $e) {
    ...
    public function options(OptionRequestData $requestData): JsonResponse
    {
        try {
            ...
            return successResponseJson([
                'data' => OrganizationLocationOptionData::fromCollection($result),
            ]);
        } catch (Exception $e) {
            return errorResponseJson(
                ['message' => $e->getMessage()]
