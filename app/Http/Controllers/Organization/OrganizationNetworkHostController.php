...
use App\Domains\Organization\Locations\OrganizationLocationException;
use App\Domains\Organization\Locations\OrganizationLocationFormData;
use App\Domains\Organization\Networks\OrganizationNetworkHostFormData;
use App\Domains\Organization\Networks\OrganizationNetworkHostOptionData;
use App\Domains\Organization\Networks\OrganizationNetworkHostResourceData;
use App\Domains\Organization\Networks\OrganizationNetworkHostService;
...
class OrganizationNetworkHostController
{
    private string $componentDashboard = 'v1/org/network_host/dashboard';

    private string $componentManagement = 'v1/org/network_host/manage-network-host';

    private string $handlerRedirectTo = 'v1.org.network_hosts.manage:get';

    public function __construct(private OrganizationNetworkHostService $service) {}
            ...
            return inertia()->render($this->componentManagement, [
                'context' => [
                    'form' => ResponseFormData::forPrefixed(
                        data: OrganizationNetworkHostResourceData::from($model)->toArray(),
    ...
    public function addHandler(OrganizationNetworkHostFormData $data): ?RedirectResponse
    {
        try {
    ...
    public function updateHandler(string $prefixedId, OrganizationNetworkHostFormData $data): ?RedirectResponse
    {
        try {
    ...
    public function datatable(DatatableRequestData $requestData): JsonResponse
    {
        try {
            ...
            return successResponseJson([
                'data' => OrganizationNetworkHostResourceData::collect(items: $result),
            ]);
        } catch (Exception $e) {
    ...
    public function options(OptionRequestData $requestData): JsonResponse
    {
        try {
            ...
            return successResponseJson([
                'data' => OrganizationNetworkHostOptionData::fromCollection($result),
            ]);
        } catch (Exception $e) {
            return errorResponseJson(
                ['message' => $e->getMessage()]
